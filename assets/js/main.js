...
(function () {
  ...
  function openQuickView(product) {
    ...
    var thumbsHtml = imgs.map(function(src, i) {
      return '<img class="qv-thumb-v' + (i === 0 ? ' active' : '') + '" src="' + src + '" alt="' + name + ' ' + (i + 1) + '"' +
        ' onclick="var m=document.getElementById(\'qv-main-img\');if(m)m.src=this.src;' +
        'var ts=this.closest(\'.qv-thumb-col\');if(ts){ts.querySelectorAll(\'.qv-thumb-v\').forEach(function(t){t.classList.remove(\'active\');});this.classList.add(\'active\');}">';
    }).join('');

    /* sale pill */
    var salePillHtml = savePct > 0 ? '<span class="qv-sale-pill">SALE &minus;' + savePct + '%</span>' : '<span></span>';
    ...
    var atcData  = '{id:\'' + (product.id || slug) + '\',name:\'' + safeName + '\',price:' + price +
      ',originalPrice:' + (orig || price) + ',image:\'' + mainImg + '\',slug:\'' + slug + '\',category:\'' + cat + '\',quantity:qvQty}';
      ...
      content.innerHTML =
        ...
        '<div class="qv-right">' +
          '<div class="qv-right-inner">' +
            ...
            '<div class="qv-top-bar">' + salePillHtml + '</div>' +
            ...
            '<div class="qv-actions">' +
              ...
              '<button class="qv-wish-btn" type="button">' +
                ...
                'Add To Wishlist' +
              '</button>' +
              '<a href="' + base + '/products/' + slug + '/" class="qv-wish-btn" style="text-decoration:none;text-align:center;">View Full Details &rarr;</a>' +
            '</div>' +

            '<div class="qv-meta">' +
              '<div class="qv-meta-row"><span class="qv-meta-label">SKU:</span> ' + slug + '</div>' +
              '<div class="qv-meta-row"><span class="qv-meta-label">Category:</span> <span class="qv-meta-pill">' + cat + '</span></div>' +
            '</div>' +

            '<div class="qv-social">' +
              '<span class="qv-social-label">Share:</span>' +
              ...
              '</a>' +
              '<a class="qv-social-icon qv-social-icon--tw" href="https://twitter.com/intent/tweet?url=' + pageUrl + '&text=' + pageTxt + '" target="_blank" rel="noopener" aria-label="Share on X">' +
              ...
              '</a>' +
              '<a class="qv-social-icon qv-social-icon--ig" href="https://instagram.com/nrich.store" target="_blank" rel="noopener" aria-label="Follow on Instagram">' +
                '<svg viewBox="0 0 24 24" fill="currentColor"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path fill="none" stroke="white" stroke-width="1.5" d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5" stroke="white" stroke-width="2"/></svg>' +
              '</a>' +
            '</div>' +
