---
layout: product
name: "Original Wayfarer Classic"
slug: original-wayfarer-classic-rb2140
sku: RB2140-90257-50-22
category: sunglasses
category_name: Sunglasses
price: 4890
sale_price: 3690
price_label: "4,890.00"
sale_price_label: "3,690.00"
savings_label: "1,200.00"
is_new: true
is_bestseller: false
is_sale: true
stock: 10
rating: 0
review_count: 0
tags:
  - sunglasses
  - wayfarer
  - classic
  - tortoise
  - polarized
  - brown
  - sale
images:
  - "/assets/images/product/ORIGINAL WAYFARER CLASSIC Model code RB2140 90257 50 22/10.jpg"
  - "/assets/images/product/ORIGINAL WAYFARER CLASSIC Model code RB2140 90257 50 22/11.jpg"
  - "/assets/images/product/ORIGINAL WAYFARER CLASSIC Model code RB2140 90257 50 22/12.jpg"
  - "/assets/images/product/ORIGINAL WAYFARER CLASSIC Model code RB2140 90257 50 22/13.jpg"
  - "/assets/images/product/ORIGINAL WAYFARER CLASSIC Model code RB2140 90257 50 22/14.jpg"
  - "/assets/images/product/ORIGINAL WAYFARER CLASSIC Model code RB2140 90257 50 22/15.jpg"
  - "/assets/images/product/ORIGINAL WAYFARER CLASSIC Model code RB2140 90257 50 22/16.jpg"
  - "/assets/images/product/ORIGINAL WAYFARER CLASSIC Model code RB2140 90257 50 22/17.jpg"
hover_image: "/assets/images/product/ORIGINAL WAYFARER CLASSIC Model code RB2140 90257 50 22/12.jpg"
video_youtube_id: "7XLqgpeD6LQ"
colors:
  - name: Polished Tortoise
    value: "#5A321F"
    image: "/assets/images/product/ORIGINAL WAYFARER CLASSIC Model code RB2140 90257 50 22/11.jpg"
glass_colors:
  - name: Brown Polarized
    value: "#6B4B2A"
    image: "/assets/images/product/ORIGINAL WAYFARER CLASSIC Model code RB2140 90257 50 22/11.jpg"
sizes:
  - "50-22"
description_short: "Original Wayfarer Classic — polished tortoise frame with brown polarized lenses in the iconic RB2140 shape."
specs:
  - label: Model
    value: Original Wayfarer Classic
  - label: Model Code
    value: RB2140 902/57 50-22
  - label: Frame Shape
    value: Square
  - label: Frame Color
    value: Polished Tortoise
  - label: Frame Material
    value: Acetate
  - label: Temple Color
    value: Tortoise
  - label: Lens Color
    value: Brown Polarized
  - label: Lens Treatment
    value: Solid Color
  - label: Lens Category
    value: 3P
  - label: Size
    value: 50-22 mm
  - label: Lens Height
    value: 41 mm
  - label: Temple Length
    value: 150 mm
  - label: Face Coverage
    value: Standard
  - label: Bridge & Nosepads
    value: High Bridge Fit
  - label: Fit
    value: Best for most faces
description: |
  ## Original Wayfarer Classic

  The timeless Wayfarer silhouette in a polished tortoise finish, paired with brown polarized lenses for a refined classic look.

  **Product Details:**
  - Model code: RB2140 902/57 50-22
  - Square acetate frame in Polished Tortoise
  - Brown Polarized solid-color lenses (Category 3P)
  - Size: 50-22 mm
  - Lens height: 41 mm
  - Temple length: 150 mm
  - Standard face coverage with High Bridge Fit
  - One size — best for most faces
---
